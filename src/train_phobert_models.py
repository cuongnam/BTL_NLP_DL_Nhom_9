...
def get_training_args(output_dir, run_name):
    return TrainingArguments(
        ...
        load_best_model_at_end=True,    # Tự động load lại bản tốt nhất khi kết thúc
        metric_for_best_model="f1",     # Dựa trên F1-score để chọn bản tốt nhất
        greater_is_better=True,
        save_total_limit=1,             # Quan trọng: Chỉ giữ lại 1 bản checkpoint tốt nhất
        learning_rate=3e-5,
...
def get_compute_metrics(idx_to_label):
    ...
    def compute_metrics(p):
        ...
        true_predictions = [[idx_to_label[p_val] for (p_val, l_val) in zip(prediction, label) if l_val != -100]
                            for prediction, label in zip(predictions, labels)]
        true_labels = [[idx_to_label[l_val] for (p_val, l_val) in zip(prediction, label) if l_val != -100]
                       for prediction, label in zip(predictions, labels)]
...
def run_03_phobert_pure():
    ...
    trainer = Trainer(
        model=model,
        args=get_training_args("./saved_models/run_03", "run_03_phobert_trigger"),
        train_dataset=BKEEPhoBERTDataset("./data/processed_bio/train_bio.json", tokenizer, label_to_idx),
        eval_dataset=BKEEPhoBERTDataset("./data/processed_bio/dev_bio.json", tokenizer, label_to_idx),
        compute_metrics=get_compute_metrics({i: l for i, l in enumerate(label_list)}),
    )
    trainer.train()
    wandb.finish()
...
def run_04_phobert_joint():
    ...
    with open("./data/processed_joint/train_joint.json", "r", encoding="utf-8") as f:
        for line in f:
            for l in json.loads(line.strip())["labels"]: label_set.add(l)
    ...
    trainer = Trainer(
        model=model,
        args=get_training_args("./saved_models/run_04", "run_04_phobert_trigger_event_type"),
        train_dataset=BKEEPhoBERTDataset("./data/processed_joint/train_joint.json", tokenizer, label_to_idx),
        eval_dataset=BKEEPhoBERTDataset("./data/processed_joint/dev_joint.json", tokenizer, label_to_idx),
        compute_metrics=get_compute_metrics({i: l for i, l in enumerate(label_list)}),
    )
    trainer.train()
    wandb.finish()
